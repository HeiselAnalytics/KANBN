...
export function SettingsView({ initialSettings, boards, sections }: { initialSettings: AppSettings; boards: BoardSummary[]; sections: BoardSectionSummary[] }) {
  ...
  return <div className="h-[calc(100dvh-56px)] overflow-y-auto p-4 lg:h-dvh lg:p-6">
    <div className="mx-auto max-w-3xl">
      ...
      <section className="border-b border-[var(--border)] py-5"><h2 className="mb-4 mt-0 text-lg font-semibold">Appearance</h2><fieldset className="m-0 border-0 p-0"><legend className="field-label mb-2">Theme</legend><div className="flex flex-wrap gap-2">{(["light", "dark", "system"] as const).map((theme) => <label key={theme} className="flex min-h-8 cursor-pointer items-center gap-2 rounded-[6px] border border-[var(--border)] px-3 capitalize"><input type="radio" name="theme" value={theme} checked={settings.theme === theme} onChange={() => { updateSettings({ ...settings, theme }); applyTheme(theme); }} /> {theme}</label>)}</div></fieldset><label className="mt-4 flex min-h-8 items-center gap-2"><input type="checkbox" checked={settings.compactCards} onChange={(event) => updateSettings({ ...settings, compactCards: event.target.checked })} /> Compact cards</label><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="field"><span className="field-label">Logo URL · Light Mode</span><input className="input" type="url" inputMode="url" placeholder="https://example.com/logo-dark.png" value={settings.logoLightUrl} onChange={(event) => updateSettings({ ...settings, logoLightUrl: event.target.value })} maxLength={2048} /></label><label className="field"><span className="field-label">Logo URL · Dark Mode</span><input className="input" type="url" inputMode="url" placeholder="https://example.com/logo-light.png" value={settings.logoDarkUrl} onChange={(event) => updateSettings({ ...settings, logoDarkUrl: event.target.value })} maxLength={2048} /></label></div><p className="help mb-0 mt-3 text-xs">Use absolute HTTP or HTTPS image links. Leave a field empty to use the bundled Heisel Analytics logo for that theme.</p></section>
