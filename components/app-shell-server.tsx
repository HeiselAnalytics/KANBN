...
export async function AppShellServer({ currentTitle, children }: { currentTitle: string; children: ReactNode }) {
  ...
  return <AppShell boards={boards} sections={sections} templates={templates} applicationName={settings.applicationName} branding={getBrandingConfig()} projectVersion={getProjectVersion()} defaultTheme={settings.theme} language={settings.language} currentTitle={currentTitle}>{children}</AppShell>;
}
